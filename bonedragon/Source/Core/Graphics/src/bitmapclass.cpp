...
bool BitmapClass::Initialize(ID3D10Device* device, int screenWidth, int screenHeight, WCHAR* textureFilename, 
	int bitmapWidth, int bitmapHeight)
{
	...
	//Initialize the previous rendering postion to negative one.
	m_previousPosX = -1;
	m_previousPosY = -1;
...
bool BitmapClass::InitializeBuffers(ID3D10Device* device)
{
	...
	// Give the subresource structe a point to the vertex data
	vertexData.pSysMem = vertices;

	//Now finally crea the vertex buffer.
	result = device->CreateBuffer(&vertexBufferDesc, &vertexData, &m_vertexBuffer);
	if(FAILED(result)){
		return false;
	}

	//Set up the descritiption of the index buffer
	...
	//Give the subreasource structure a pointer to the index data.
	indexData.pSysMem = indices;

	//Crea the index buffer
...
bool BitmapClass::UpdateBuffers(int positionX, int positionY)
{
	...
	// Calculate the screen coordinates of the rigfht side of the bitmap.
