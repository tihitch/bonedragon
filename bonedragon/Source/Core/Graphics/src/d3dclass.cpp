							...
							bool fullscreen, float screenDepth, float screenNear)
{
	...
	//Create a DirectX graphics inter
	result = CreateDXGIFactory(__uuidof(IDXGIFactory), (void **)&factory);
	if(FAILED(result)){
		return false;
	}
	//Use the factory to crea an adapter for the primary graphics interface (GPU)
	...
	//Now go through all the display modes nad find the one that matches the screen width and height.
	...
	result = D3D10CreateDeviceAndSwapChain(NULL, D3D10_DRIVER_TYPE_HARDWARE, NULL,
		0, D3D10_SDK_VERSION, &swapChainDesc, &m_swapChain, &m_device);
	if(FAILED(result))
	{
		return false;
	}
	...
	// Create the texture for the depth buffer using the filled out descriptioon
	...
	//Create the raster rasterizer state from the description we just filled out.
...
{
	//Present the back buffer to the screen since redering is complete
	if(m_vsync_enabled)
	{
